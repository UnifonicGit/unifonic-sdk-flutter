...
class PushHostHandlers: NSObject, UNIPushHostApi {
    ...
    func getNotificationSettings(completion: @escaping (UNIUNNotificationSettings?, FlutterError?) -> Void) {
        UNUserNotificationCenter.current().getNotificationSettings { settings in
            completion(UNIUNNotificationSettings.from(unSettings: settings), nil)
        }
    }

    private let delegate: UNUserNotificationCenterDelegate
    private let pushFlutterApi: UNIPushFlutterApi
    ...
    init(binaryMessenger: FlutterBinaryMessenger, originalDelegate: UNUserNotificationCenterDelegate?) {
        pushFlutterApi = UNIPushFlutterApi(binaryMessenger: binaryMessenger)
        ...
        SetUpUNIPushHostApi(binaryMessenger, self)
    }
                                      ...
                                      fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) -> Bool
    {
        ...
        let message = UNIRemoteMessage.from(userInfo: userInfo)
